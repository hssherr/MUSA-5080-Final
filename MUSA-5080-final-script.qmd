...
crime_subdirs <- grep("crime[0-9]{4}", crime_dirs, value = TRUE)

# 2. Find all .shp files inside those directories
shapefiles <- list.files(crime_dirs, pattern = "\\.shp$", full.names = TRUE)

# 3. Read all shapefiles into a list
shp_list <- lapply(shapefiles, st_read)
```
...
crime_tract <- crime_tract %>% 
  mutate(month = format(dispatch_date_time, format = "%m/%Y"))
...
data_model <- list(
  evictions %>% st_drop_geometry() %>% select(GEOID, filings_2020, month_date),
  housing_cnts %>% 
)
```
